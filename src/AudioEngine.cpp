...
void AudioEngine::unload(PluginHost* pluginToUnload)
{
    ...
    QTimer::singleShot(500, this, [this, indexOfPluginToRemove, oldStatus, pluginToUnload]()
    {
        ...
        QTimer::singleShot(500, this, [this, oldStatus]()
        {
            m_status = oldStatus;
        });
    });
...
void AudioEngine::load(PluginHost* plugin, const QString& path, const int pluginIndex)
{
    ...
    auto loadPlugin = [this, oldStatus, plugin, path, pluginIndex]() -> void
    {
        if (isRunning())
        {
            qWarning() << "still running, it shouldn't";
            return;
        }
    ...
    if (m_status > ocp::Status::OnHold)
    {
        connect(this, &AudioEngine::isRunningChanged, this, loadPlugin, Qt::SingleShotConnection);
        stop();
...
int AudioEngine::audioCallback(void* outputBuffer, void* inputBuffer, const unsigned int frameCount,
                               ...
                               void* data)
{
    ...
    if (in)
    {
        if (engine->m_inputChannelCount == 1)
        {
            for (unsigned int i = 0; i < frameCount; ++i)
            {
                engine->m_outputBuffer[0][i] = in[i];
                engine->m_outputBuffer[1][i] = in[i];
            }
        }
        else
        {
            for (unsigned int i = 0; i < frameCount; ++i)
            {
                ...
                for (int j = 0; j < engine->m_inputChannelCount; ++j)
                {
                    engine->m_outputBuffer[0][i] += in[(2 * i) + j];
                    engine->m_outputBuffer[1][i] += in[(2 * i) + j];
                }
            }
        }
    }
