...
void PluginHost::setIsFloatingWindowOpen(const bool value)
{
    if (value)
    {
        ...
        {
            qDebug() << "Closing floating window for: " << m_name;
...
void PluginHost::setIsNativeGuiOpen(const bool value)
{
    if (value == m_isNativeGuiOpen)
        return;

    m_isNativeGuiOpen = value;
    emit isNativeGuiOpenChanged();
}
...
void PluginHost::process()
{
    ...
    if (m_status != ocp::Status::Running || m_isByPassed)
        return;
...
void PluginHost::createGuiWindow(QQuickWindow* parentWindow)
{
    ...
    parentWindow->resize(static_cast<int>(width), static_cast<int>(height));
    ...
    QTimer::singleShot(100, this, [this]()
    {
        ...
        if (!m_plugin->guiGetSize(&_width, &_height))
        {
            qWarning() << "could not get the size of the plugin gui";
            m_isNativeGuiOpen = false;
            m_plugin->guiDestroy();
        ...
        m_parentWindow->resize(static_cast<int>(_width), static_cast<int>(_height));
        emit isNativeGuiOpenChanged();
        emit guiSizeChanged();
    });
}
